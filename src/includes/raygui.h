...
#define AUTO_CURSOR_DELAY 1     // Frames delay for autocursor movement
    ...
    else if (state == STATE_DISABLED)
    {
        ...
        GuiDrawRectangle(bounds, GuiGetStyle(TEXTBOX, BORDER_WIDTH), Fade(GetColor(GuiGetStyle(TEXTBOX, BORDER + (state * 3))), guiAlpha), BLANK);
    ...
    if ((text != NULL) && (text[0] != '\0'))
    {
        ...
        for (int i = 0; i < lineCount; i++)
        {
            ...
            case 1:
                ...
                break; // UP
            case 2:
                position.y = bounds.y + posOffsetY + bounds.height - totalHeight + TEXT_VALIGN_PIXEL_OFFSET(bounds.height);
                break; // DOWN
            default:
                break;
            }
    ...
    if ((state != STATE_DISABLED) && !guiLocked)
    {
        ...
        if (guiSliderDragging) // Keep dragging outside of bounds
        {
            if (IsMouseButtonDown(MOUSE_LEFT_BUTTON))
            {
                ...
                {
                    if (isVertical)
                        value += (int)(GetMouseDelta().y / (scrollbar.height - slider.height) * valueRange);
                    else
                        value += (int)(GetMouseDelta().x / (scrollbar.width - slider.width) * valueRange);
                }
            }
            else
            {
